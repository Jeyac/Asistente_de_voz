...
class TextVectorizer:
    ...
    def __init__(self, max_tokens: int, sequence_length: int) -> None:
        ...
        self._layer = TextVectorization(max_tokens=max_tokens, output_mode='int', output_sequence_length=sequence_length)
    ...
    def transform_one(self, text: str) -> np.ndarray:
        """Función transform one."""
        return self.transform([text])[0]

    def save(self, path: Path) -> None:
        """Guarda datos o artefactos en disco."""
        payload = {'max_tokens': self._max_tokens, 'sequence_length': self._sequence_length, 'vocabulary': self.get_vocabulary()}
    ...
    def load(cls, path: Path) -> 'TextVectorizer':
        ...
        vectorizer = cls(max_tokens=int(payload['max_tokens']), sequence_length=int(payload['sequence_length']))
