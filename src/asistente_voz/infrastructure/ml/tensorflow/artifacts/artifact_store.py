...
class ArtifactStore:
    ...
    def save(self, model: keras.Model, pipeline: NlpPipeline, metrics: TrainingMetrics, classification_report: dict[str, Any]) -> None:
        ...
        metadata = {'metrics': asdict(metrics), 'classification_report': classification_report, 'confidence_threshold': self._settings.tf_confidence_threshold, 'fallback_intent': self._settings.tf_fallback_intent, 'labels': pipeline.label_encoder.labels}
        self._paths.metadata_file.write_text(json.dumps(metadata, ensure_ascii=False, indent=2), encoding='utf-8')
        logger.info('Artefactos guardados en %s', self._paths.models_dir)

    def load_metadata(self) -> dict[str, Any]:
        """Función load metadata."""
