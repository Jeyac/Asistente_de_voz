...
from asistente_voz.infrastructure.ml.tensorflow.model.architecture import (
    IntentClassifierModelFactory,
)
...
from asistente_voz.infrastructure.ml.tensorflow.training.dataset_builder import (
...
class IntentModelTrainer:
    ...
    def __init__(
        self,
        settings: Settings,
        intents_repository: IntentsRepository,
    ...
    def train(self) -> TrainingMetrics:
        ...
        model = self._model_factory.build(
            vocab_size=len(pipeline.vectorizer.get_vocabulary()),
            num_classes=pipeline.label_encoder.num_classes,
            sequence_length=self._settings.tf_sequence_length,
            embedding_dim=self._settings.tf_embedding_dim,
        )
        model.compile(
            optimizer=keras.optimizers.Adam(learning_rate=self._settings.tf_learning_rate),
            loss="sparse_categorical_crossentropy",
            metrics=["accuracy"],
        )

        num_samples = len(texts)
        # Con pocos ejemplos, val_split deja demasiado pocos datos y early stopping corta mal el entrenamiento
        ...
        if use_validation:
            ...
            logger.info(
                "Dataset pequeño (%s muestras): entrenamiento sin validación ni early stopping",
                num_samples,
            )
        ...
        history = model.fit(
            features,
            encoded_labels,
            epochs=self._settings.tf_epochs,
            batch_size=min(self._settings.tf_batch_size, max(1, num_samples // 4)),
        ...
        classification_report = self._evaluator.classification_report(
            model,
            features,
            encoded_labels,
        ...
        logger.info(
            "Entrenamiento finalizado | accuracy=%.4f | val_accuracy=%.4f",
            metrics.accuracy,
    ...
    def _build_pipeline(self) -> NlpPipeline:
        vectorizer = TextVectorizer(
            max_tokens=self._settings.tf_max_vocab_size,
            sequence_length=self._settings.tf_sequence_length,
        )
        return NlpPipeline(
            cleaner=TextCleaner(),
