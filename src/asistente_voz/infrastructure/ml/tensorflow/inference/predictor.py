...
class IntentPredictor:
    ...
    def __init__(self, model: keras.Model, pipeline: NlpPipeline, confidence_threshold: float, fallback_intent: str) -> None:
    ...
    def predict(self, text: str) -> PredictionResult:
        ...
        probability_map = {self._pipeline.label_encoder.decode(index): float(score) for index, score in enumerate(probabilities)}
        return PredictionResult(intent=intent, confidence=confidence, raw_text=text, cleaned_text=cleaned, above_threshold=above_threshold, probabilities=probability_map)
