    ...
    def load(self) -> LoadedIntentModel:
        if not self.is_available:
            missing = [
                ...
                for path in (
                    ...
                    self._paths.tokenizer_file,
                    self._paths.labels_file,
                    self._paths.metadata_file,
                )
                if not path.is_file()
            ]
            raise FileNotFoundError(
                f"Artefactos incompletos. Archivos faltantes: {missing}",
            )
        ...
        predictor = IntentPredictor(
            ...
            confidence_threshold=float(
                metadata.get("confidence_threshold", self._settings.tf_confidence_threshold),
            ),
            fallback_intent=str(
                metadata.get("fallback_intent", self._settings.tf_fallback_intent),
            ),
        )
        logger.info(
            "Modelo cargado | clases=%s | path=%s",
