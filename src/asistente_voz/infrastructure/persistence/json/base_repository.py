...
class JsonFileRepository:
    ...
    def read(self) -> dict[str, Any]:
        """Función read."""
        if not self.exists():
            raise NotFoundError(message=f'Archivo no encontrado: {self._file_path}', details={'path': str(self._file_path)})
        try:
            with self._file_path.open(encoding='utf-8') as file:
                data = json.load(file)
        except json.JSONDecodeError as exc:
            raise ValidationError(message=f'JSON inválido en {self._file_path}', details={'error': str(exc)}) from exc
