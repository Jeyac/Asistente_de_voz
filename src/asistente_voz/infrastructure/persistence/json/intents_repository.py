        ...
        for item in raw_intents:
            ...
            if not isinstance(examples, list) or not examples:
                raise ValidationError(
                    message=f"La intención '{name}' requiere ejemplos.",
                )
            cleaned_examples = tuple(
                str(example).strip() for example in examples if str(example).strip()
            )
            if not cleaned_examples:
                raise ValidationError(
                    message=f"La intención '{name}' no tiene ejemplos válidos.",
                )
            definitions.append(
                IntentDefinition(name=name.strip(), examples=cleaned_examples),
            )
        return definitions
