...
class SmartResponseService(IResponseProvider):
    ...
    async def generate_from_prediction(self, prediction: PredictionResult) -> SmartResponse:
        ...
        return await self.generate_for_intent(
            intent_name=intent_name,
            confidence=prediction.confidence,
            above_threshold=prediction.above_threshold,
            used_fallback=not prediction.above_threshold,
        )
    ...
    async def generate_for_intent(
        ...
        *,
        ...
        logger.info(
            "Respuesta generada | intent=%s | variants=%s | fallback=%s",
            intent_responses.intent,
            intent_responses.count,
            used_fallback,
        )
        return SmartResponse(
            intent=intent_responses.intent,
            message=message,
            confidence=confidence,
            above_threshold=above_threshold,
            variants_available=intent_responses.count,
    ...
    def _resolve_intent(self, catalog: ResponseCatalog, intent_name: str) -> IntentResponses:
        ...
        raise ValidationError(
            message=f"No hay respuestas para '{intent_name}' ni para el fallback '{fallback_name}'.",
        )
    ...
    def _build_message(self, intent_name: str, variants: list[str]) -> str:
        ...
        if intent_name == "relacion_creador":
            ...
            return self._selector.select(variants).format(
                pareja=partner,
                titulo=title,
                creadora=creadora,
            )
    ...
    def _validate_catalog(catalog: ResponseCatalog) -> None:
        ...
        for intent in catalog.intents:
            if intent.enabled and not intent.variants:
                raise ValidationError(
                    message=f"La intención '{intent.intent}' no tiene variantes configuradas.",
                )
