...
class SpeechRecognitionService(ISpeechService):
    ...
    async def listen_from_microphone(self) -> VoiceCommand:
        ...
        logger.info(
            "Escuchando micrófono | timeout=%ss | phrase_limit=%ss",
            self._settings.speech_recognition_timeout,
            self._settings.speech_recognition_phrase_limit,
        )
        return await asyncio.to_thread(
            self._listen_microphone_sync,
            timeout=self._settings.speech_recognition_timeout,
            phrase_limit=self._settings.speech_recognition_phrase_limit,
            ambient_duration=self._settings.speech_ambient_noise_duration,
        )
    ...
    async def listen_command_after_activation(self) -> VoiceCommand:
        ...
        logger.info(
            "Escuchando comando post-activación | timeout=%ss | phrase_limit=%ss | ambient=%ss",
            self._settings.speech_post_wakeword_timeout,
            self._settings.speech_post_wakeword_phrase_limit,
            self._settings.speech_post_wakeword_ambient_duration,
        )
        return await asyncio.to_thread(
            self._listen_microphone_sync,
            timeout=self._settings.speech_post_wakeword_timeout,
            phrase_limit=self._settings.speech_post_wakeword_phrase_limit,
            ambient_duration=self._settings.speech_post_wakeword_ambient_duration,
        )
