...
from asistente_voz.infrastructure.wakeword.audio_stream import CHANNELS, FORMAT_WIDTH, SAMPLE_RATE, MicrophoneChunkStream
...
class PorcupineDetector(IWakeWordDetector):
    ...
    def _initialize(self):
        ...
        if not access_key:
            raise WakeWordEngineError(message='PORCUPINE_ACCESS_KEY no configurada.', details={'hint': 'Obtenga una clave en https://console.picovoice.ai/'})
        if not keyword_path.is_file():
            raise WakeWordModelNotFoundError(message=f'Archivo .ppn no encontrado: {keyword_path}', details={'hint': "Cree la palabra clave 'oye sistema' en la consola de Picovoice."})
        try:
            import pvporcupine
        except ImportError as exc:
            raise WakeWordEngineError(message='Porcupine no está instalado.', details={'install': 'pip install pvporcupine'}) from exc
        self._porcupine = pvporcupine.create(access_key=access_key, keyword_paths=[str(keyword_path)])
    ...
    def _listen_sync(self, timeout: float) -> WakeWordActivation:
        ...
        logger.info("Escuchando palabra clave (Porcupine) | phrase='%s' | timeout=%ss", self._settings.wakeword_phrase, timeout)
        import pyaudio
        pa = pyaudio.PyAudio()
        stream = pa.open(rate=porcupine.sample_rate, channels=CHANNELS, format=pyaudio.paInt16, input=True, frames_per_buffer=frame_length)
        try:
            ...
            while time.perf_counter() < deadline:
                ...
                if keyword_index >= 0:
                    ...
                    activation = WakeWordActivation.now(keyword=self._settings.wakeword_phrase, engine=self.engine_name, confidence=1.0, latency_ms=latency_ms)
                    logger.info("¡Palabra clave detectada (Porcupine)! | phrase='%s' | latency_ms=%.0f", activation.keyword, activation.latency_ms or 0)
                    return activation
        finally:
            stream.stop_stream()
            stream.close()
