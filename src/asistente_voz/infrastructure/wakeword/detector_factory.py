...
class WakeWordDetectorFactory:
    ...
    def create(settings: Settings) -> IWakeWordDetector:
        ...
        if engine == 'openwakeword':
            from asistente_voz.infrastructure.wakeword.openwakeword_detector import OpenWakeWordDetector
            return OpenWakeWordDetector(settings)
        if engine == 'porcupine':
            from asistente_voz.infrastructure.wakeword.porcupine_detector import PorcupineDetector
            return PorcupineDetector(settings)
        raise WakeWordEngineError(message=f'Motor de palabra clave no soportado: {engine}', details={'supported': ['openwakeword', 'porcupine']})
