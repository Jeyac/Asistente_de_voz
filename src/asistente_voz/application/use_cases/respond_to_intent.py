...
class RespondToIntentUseCase:
    ...
    def __init__(self, classify_intent: ClassifyIntentUseCase, response_provider: IResponseProvider) -> None:
    ...
    async def execute(self, text: str) -> IntentReply:
        ...
        logger.info('Respuesta inteligente | intent=%s | message=%s', smart_response.intent, smart_response.message[:50])
        return IntentReply(transcript=text, prediction_intent=prediction.intent, confidence=prediction.confidence, above_threshold=prediction.above_threshold, cleaned_text=prediction.cleaned_text, probabilities=prediction.probabilities, response=smart_response)
