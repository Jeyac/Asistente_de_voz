...
Flujo: transcripción (texto/WAV/micrófono) → clasificación TensorFlow →
respuesta aleatoria desde JSON → URL de acción opcional para el navegador.
"""
...
class ProcessVoiceCommandUseCase:
    ...
    def __init__(self, speech_service: ISpeechService, classify_intent: ClassifyIntentUseCase, response_provider: IResponseProvider, action_executor: IIntentActionExecutor) -> None:
    ...
    async def execute_from_microphone(self) -> VoiceProcessResult:
        """Función execute from microphone."""
    ...
    async def _process_command(self, command: VoiceCommand) -> VoiceProcessResult:
        """Método interno: process command."""
        ...
        if smart_response.above_threshold and (not smart_response.used_fallback):
            action = await self._action_executor.execute(smart_response.intent, command.transcript)
            if action and action.url:
                action_url = action.url
                action_performed = action.performed
                message = f'{message} Te lo abro en una nueva pestaña.'
        logger.info('Comando procesado | intent=%s | confidence=%.4f | action=%s', smart_response.intent, smart_response.confidence, action_performed)
        return VoiceProcessResult(transcript=command.transcript, language=command.language, intent=smart_response.intent, confidence=smart_response.confidence, above_threshold=smart_response.above_threshold, response=message, cleaned_text=prediction.cleaned_text, probabilities=prediction.probabilities, action_performed=action_performed, action_url=action_url)
