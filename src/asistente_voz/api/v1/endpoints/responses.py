...
from asistente_voz.api.dependencies import (
    GenerateSmartResponseUseCaseDep,
    RespondToIntentUseCaseDep,
    ResponseServiceDep,
)
from asistente_voz.domain.entities.smart_response import SmartResponse
from asistente_voz.schemas.response import (
...
def _to_schema(smart: SmartResponse) -> SmartResponseSchema:
    return SmartResponseSchema(
        intent=smart.intent,
        message=smart.message,
        confidence=round(smart.confidence, 4),
        above_threshold=smart.above_threshold,
        variants_available=smart.variants_available,
        used_fallback=smart.used_fallback,
    )
...
@router.post(
    ...
    summary="Clasificar y responder",
    description="Clasifica el texto con TensorFlow y devuelve una respuesta aleatoria del catálogo JSON.",
)
async def generate_response(
    payload: GenerateResponseRequest,
    use_case: RespondToIntentUseCaseDep,
) -> GenerateResponseResult:
    ...
    return GenerateResponseResult(
        ...
        response=_to_schema(result.response),
    )


@router.get(
    "/catalog",
    response_model=ResponseCatalogSchema,
    summary="Catálogo de respuestas",
)
async def get_catalog(service: ResponseServiceDep) -> ResponseCatalogSchema:
    ...
    return ResponseCatalogSchema(
        ...
        default_intent=catalog.default_intent,
        intents=[
            IntentCatalogItem(
                intent=item.intent,
                variants_count=item.count,
...
@router.get(
    "/{intent_name}",
    response_model=SmartResponseSchema,
    summary="Respuesta aleatoria por intención",
)
async def get_response_for_intent(
    intent_name: str,
    use_case: GenerateSmartResponseUseCaseDep,
) -> SmartResponseSchema:
