...
from asistente_voz.schemas.voice import (
...
def _to_process_response(result: VoiceProcessResult) -> VoiceProcessResponse:
    return VoiceProcessResponse(
        ...
        probabilities={key: round(value, 4) for key, value in result.probabilities.items()},
...
@router.post(
    ...
    summary="Procesar texto de voz",
    description="Clasifica la intención y genera respuesta a partir de texto transcrito.",
)
async def process_voice_text(
    payload: VoiceTextRequest,
    use_case: ProcessVoiceUseCaseDep,
) -> VoiceProcessResponse:
...
@router.post(
    ...
    description="Convierte un archivo WAV a texto en español.",
)
async def transcribe_audio(
    use_case: ProcessVoiceUseCaseDep,
    validator: AudioValidatorDep,
    audio: UploadFile = File(..., description="Archivo WAV con el comando de voz"),
) -> VoiceTranscribeResponse:
...
@router.post(
    ...
    response_model=VoiceProcessResponse,
    status_code=status.HTTP_200_OK,
    summary="Procesar archivo de audio",
    description="Transcribe audio, clasifica intención con TensorFlow y devuelve respuesta.",
)
async def process_voice_audio(
    ...
    validator.validate_upload(audio.filename, audio.content_type, content)
    logger.info(
        "Procesando audio completo | filename=%s | size=%s",
        audio.filename,
        len(content),
    )
...
@router.post(
    ...
    response_model=VoiceProcessResponse,
    status_code=status.HTTP_200_OK,
    summary="Escuchar micrófono y procesar",
    description=(
        "Captura audio desde el micrófono local, transcribe y clasifica la intención. "
        "Requiere entorno con dispositivo de audio (no disponible en Render)."
    ),
)
async def listen_and_process(
    ...
    if settings.is_production:
        ...
        raise ValidationError(
            message="La captura por micrófono no está habilitada en producción.",
            details={"hint": "Use /voice/process-audio con un archivo WAV."},
        )
