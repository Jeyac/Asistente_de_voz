...
from asistente_voz.schemas.intent import (
...
@router.post(
    "/predict",
    response_model=IntentPredictResponse,
    status_code=status.HTTP_200_OK,
    summary="Predecir intención",
    description="Clasifica el texto del usuario usando el modelo TensorFlow entrenado.",
)
async def predict_intent(
    payload: IntentPredictRequest,
    use_case: ClassifyIntentUseCaseDep,
) -> IntentPredictResponse:
...
async def model_status(settings: SettingsDep) -> ModelStatusResponse:
    ...
    return ModelStatusResponse(
        ...
        model_path=str(paths.model_file) if registry.is_loaded else None,
    )
