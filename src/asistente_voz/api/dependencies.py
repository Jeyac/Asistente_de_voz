"""
Inyección de dependencias para la capa API (FastAPI Depends).

Centraliza factories y casos de uso para que los endpoints no
instancien servicios manualmente.
"""
...
def get_response_factory(settings: SettingsDep) -> ResponseFactory:
    """Obtiene response factory."""
    return ResponseFactory(settings)
ResponseFactoryDep = Annotated[ResponseFactory, Depends(get_response_factory)]

def get_voice_factory(settings: SettingsDep, ml_factory: MlFactoryDep) -> VoiceFactory:
    """Obtiene voice factory."""
...
def get_classify_intent_use_case(factory: MlFactoryDep) -> ClassifyIntentUseCase:
    """Obtiene classify intent use case."""
...
def get_response_service(factory: ResponseFactoryDep) -> SmartResponseService:
    """Obtiene response service."""
    return factory.create_response_service()
ResponseServiceDep = Annotated[SmartResponseService, Depends(get_response_service)]

def get_generate_smart_response_use_case(factory: ResponseFactoryDep) -> GenerateSmartResponseUseCase:
    """Obtiene generate smart response use case."""
    return factory.create_generate_smart_response_use_case()
GenerateSmartResponseUseCaseDep = Annotated[GenerateSmartResponseUseCase, Depends(get_generate_smart_response_use_case)]

def get_respond_to_intent_use_case(ml_factory: MlFactoryDep, response_factory: ResponseFactoryDep) -> RespondToIntentUseCase:
    """Obtiene respond to intent use case."""
    return response_factory.create_respond_to_intent_use_case(ml_factory.create_classify_use_case())
...
def get_process_voice_use_case(voice_factory: VoiceFactoryDep) -> ProcessVoiceCommandUseCase:
    """Obtiene process voice use case."""
...
def get_audio_validator(voice_factory: VoiceFactoryDep) -> AudioValidator:
    """Obtiene audio validator."""
    return voice_factory.create_audio_validator()
AudioValidatorDep = Annotated[AudioValidator, Depends(get_audio_validator)]

def get_wakeword_factory(settings: SettingsDep, voice_factory: VoiceFactoryDep) -> WakeWordFactory:
    """Obtiene wakeword factory."""
    return WakeWordFactory(settings, voice_factory)
WakeWordFactoryDep = Annotated[WakeWordFactory, Depends(get_wakeword_factory)]

def get_activate_and_listen_use_case(factory: WakeWordFactoryDep) -> ActivateAndListenUseCase:
    """Obtiene activate and listen use case."""
    return factory.create_activate_and_listen_use_case()
ActivateAndListenUseCaseDep = Annotated[ActivateAndListenUseCase, Depends(get_activate_and_listen_use_case)]
