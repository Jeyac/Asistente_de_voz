...
class WakeWordConfig:
    ...
    wakeword_engine: Literal['openwakeword', 'porcupine'] = Field(default='openwakeword', alias='WAKEWORD_ENGINE')
    wakeword_phrase: str = Field(default='oye sistema', alias='WAKEWORD_PHRASE')
    wakeword_model_path: Path = Field(default=Path('models/wakeword/oye_sistema.onnx'), alias='WAKEWORD_MODEL_PATH')
    wakeword_threshold: float = Field(default=0.5, alias='WAKEWORD_THRESHOLD')
    wakeword_listen_timeout: float = Field(default=60.0, alias='WAKEWORD_LISTEN_TIMEOUT')
    wakeword_chunk_duration_ms: int = Field(default=80, alias='WAKEWORD_CHUNK_DURATION_MS')
    porcupine_access_key: str = Field(default='', alias='PORCUPINE_ACCESS_KEY')
    porcupine_keyword_path: Path = Field(default=Path('models/wakeword/oye_sistema_es_windows.ppn'), alias='PORCUPINE_KEYWORD_PATH')
