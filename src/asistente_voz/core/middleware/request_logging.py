...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        logger.info(
            "Petición entrante | id=%s | method=%s | path=%s | client=%s",
            request_id,
            request.method,
            request.url.path,
            client_host,
        ...
        try:
            ...
            logger.exception(
                "Petición fallida | id=%s | method=%s | path=%s | duration_ms=%.2f",
                request_id,
                request.method,
                request.url.path,
                duration_ms,
        ...
        log_fn(
            "Petición completada | id=%s | method=%s | path=%s | status=%s | duration_ms=%.2f",
            request_id,
            request.method,
            request.url.path,
